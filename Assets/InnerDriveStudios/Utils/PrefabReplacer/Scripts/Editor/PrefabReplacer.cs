...
namespace InnerDriveStudios.Util
{
	...
	public class PrefabReplacer : EditorWindow
	{
		...
		private void OnGUI()
		{
			...
			{
				helpText += "Select 1 or more objects to be replaced.\n";
			}

			if (settings.replacers == null || settings.replacers.Length == 0)
			{
				helpText += "Add 1 or more replacer prefabs to the list.\n";
			}

			if (helpText != "")
			{
				GUILayout.Box(helpText, GUILayout.ExpandWidth(true));
			}

			EditorGUILayout.BeginHorizontal();
			GUILayout.FlexibleSpace();

			//if there was no helpText everything is ok :)
			GUI.enabled = (helpText == "");
