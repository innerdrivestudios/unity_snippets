...
namespace InnerDriveStudios.Util
{
    ...
    public static class TransformExtensions
    {
         ...
         */
        public static void SnapToGrid(this Transform pTransform, float pGridSize)
        {
            // Calculate the new position by rounding the transform's position to the nearest multiple of the grid size
            Vector3 newPosition = pTransform.position;
         ...
         */
        public static Transform[] GetChildren(this Transform pTransform)
		{
            int childCount = pTransform.childCount;
            Transform[] children = new Transform[childCount];
            for (int i = 0; i < childCount; i++) children[i] = pTransform.GetChild(i);
            return children;
		}
    }
}
